...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes gateway` should use `hermes_cli.main`, not the legacy Fire wrapper."""
    ...
    def legacy_cli_main(*args, **kwargs):  # pragma: no cover - defensive only
    ...
    def legacy_fire(*args, **kwargs):  # pragma: no cover - defensive only
